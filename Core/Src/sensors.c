...
HAL_StatusTypeDef hts221_init(I2C_HandleTypeDef *handler)
{
  ...
  status = HAL_I2C_Mem_Read(handler, HTS221_READ, HTS221_WHO_AM_I, 1, buffer, 1, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  if (buffer[0] == 0xBC)
  {
    ...
    status = HAL_I2C_Mem_Write(handler, HTS221_WRITE, HTS221_AV_CONF, 1, buffer, 1, SENSOR_COM_TIMEOUT);
    if (status != HAL_OK)
    {
      return status;
    }
    ...
    status = HAL_I2C_Mem_Write(handler, HTS221_WRITE, HTS221_CTRL_REG, 1, buffer, 3, SENSOR_COM_TIMEOUT);
    if (status != HAL_OK)
    {
      return status;
    }
...
HAL_StatusTypeDef hts221_read(I2C_HandleTypeDef *handler, hts221_typedef *data)
{
  ...
  status = HAL_I2C_Mem_Read(handler, HTS221_READ, HTS221_DATA_OUT, 1, buffer, 5, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  status = HAL_I2C_Mem_Read(handler, HTS221_READ, HTS221_CALIBRATION_REG, 1, buffer, 16, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
...
HAL_StatusTypeDef lis3mdl_init(I2C_HandleTypeDef *handler)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LIS3MDL_READ, LIS3MDL_WHO_AM_I, 1, buffer, 1, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  if (buffer[0] == 0x3D)
  {
    ...
    status = HAL_I2C_Mem_Write(handler, LIS3MDL_WRITE, LIS3MDL_CTRL_REG, 1, buffer, 5, SENSOR_COM_TIMEOUT);
    if (status != HAL_OK)
    {
      return status;
    }
...
HAL_StatusTypeDef lis3mdl_read(I2C_HandleTypeDef *handler, lis3mdl_typedef *data)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LIS3MDL_READ, LIS3MDL_DATA_OUT, 1, buffer, 9, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
...
HAL_StatusTypeDef lps22hb_init(I2C_HandleTypeDef *handler)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LPS22HB_READ, LPS22HB_WHO_AM_I, 1, buffer, 1, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  if (buffer[0] == 0xB1)
  {
    ...
    buffer[2] = 0x00;
    status = HAL_I2C_Mem_Write(handler, LPS22HB_WRITE, LPS22HB_CTRL_REG, 1, buffer, 3, SENSOR_COM_TIMEOUT);
    if (status != HAL_OK)
    {
      return status;
    }
...
HAL_StatusTypeDef lps22hb_read(I2C_HandleTypeDef *handler, lps22hb_typedef *data)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LPS22HB_READ, LPS22HB_DATA_OUT, 1, buffer, 4, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
...
HAL_StatusTypeDef lsm6dsl_init(I2C_HandleTypeDef *handler)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LSM6DSL_READ, LSM6DSL_WHO_AM_I, 1, buffer, 1, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  if (buffer[0] == 0x6A)
  {
    ...
    status = HAL_I2C_Mem_Write(handler, LSM6DSL_WRITE, LSM6DSL_CTRL_REG, 1, buffer, 3, SENSOR_COM_TIMEOUT);
    if (status != HAL_OK)
    {
      return status;
    }
...
HAL_StatusTypeDef lsm6dsl_read(I2C_HandleTypeDef *handler, lsm6dsl_typedef *data)
{
  ...
  status = HAL_I2C_Mem_Read(handler, LSM6DSL_READ, LSM6DSL_DATA_OUT, 1, buffer, 16, SENSOR_COM_TIMEOUT);
  if (status != HAL_OK)
  {
    return status;
  }
